...
def train_evaluate_no_conf_matrix(X_train, Y_train,  X_val, Y_val, X_test, n_outputs, num_batch_size, num_epochs, class_names, 
                   target_names):
    print('Training...')
    model = make_model_CNN(X_train.shape[1], X_train.shape[2], n_outputs )
    print(X_train.shape[0], X_train.shape[1])
    ...
    Y_train = Y_train.reshape(-1, 1)
    print(X_train.shape, Y_train.shape, X_test.shape, X_val.shape, Y_val.shape)
    le = LabelEncoder()
    Y_train = to_categorical(le.fit_transform(Y_train)) 
    ...
    start = datetime.now()
    model.fit(X_train, Y_train, batch_size=num_batch_size, epochs=num_epochs,
                    validation_data=(X_val, Y_val), verbose=1, callbacks = [earlystopping_cb, mdlcheckpoint_cb])
    duration = datetime.now() - start
    print("Training completed in time: ", duration)
    # Evaluating the model on the training and testing set
    score1 = model.evaluate(X_train, Y_train, verbose=1)
    ...
    print("Testing Accuracy: ", score[1])
     #predicting
    Y_pred = model.predict(X_test, steps=len(X_test), verbose = 0)
